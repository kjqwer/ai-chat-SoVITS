...
# Character reference audio management functions
                            ...
                            if audio_file.endswith('.wav'):
                                ...
                                if match:
                                    ...
                                    character_data[character_name]["emotions"][lang_dir].append({
                                        ...
                                        "text": text,
                                        "path": audio_path,
                                        "display_name": f"[{emotion}] {text[:30]}{'...' if len(text) > 30 else ''}"
                                    })
                            ...
                            if os.path.exists(lab_path):
                                try:
                                    ...
                                    character_data[character_name]["randoms"][lang_dir].append({
                                        "text": text,
                                        "path": wav_path,
                                        "display_name": f"[Random] {text[:30]}{'...' if len(text) > 30 else ''}"
                                    })
...
def get_character_audio_choices(character_name, character_data, language="中文"):
    """获取指定角色和语言的音频选择列表"""
    choices = []
    if character_name in character_data:
        # 添加emotions音频
        if language in character_data[character_name]["emotions"]:
            for audio_info in character_data[character_name]["emotions"][language]:
                choices.append(audio_info["display_name"])
        
        # 添加randoms音频
        if language in character_data[character_name]["randoms"]:
            for audio_info in character_data[character_name]["randoms"][language]:
                choices.append(audio_info["display_name"])
    
    return choices

def get_audio_info_by_display_name(character_name, display_name, character_data, language="中文"):
    """根据显示名称获取音频信息"""
    if character_name in character_data:
        # 在emotions中查找
        if language in character_data[character_name]["emotions"]:
            for audio_info in character_data[character_name]["emotions"][language]:
                if audio_info["display_name"] == display_name:
                    return audio_info
...
character_names = [""] + sorted(character_data.keys())  # 添加空选项作为默认值
...
def on_character_change(character_name):
    ...
    if not character_name or character_name == "":
        # 清空选择
        return (
            {"__type__": "update", "choices": [], "value": ""},  # audio_dropdown
            {"__type__": "update", "value": None},  # inp_ref
            {"__type__": "update", "value": ""},  # prompt_text
        )
    
    # 获取该角色的音频选择
    audio_choices = get_character_audio_choices(character_name, character_data, "中文")
    ...
    if default_audio:
        return (
            {"__type__": "update", "choices": audio_choices, "value": default_audio["display_name"]},  # audio_dropdown
            {"__type__": "update", "value": default_audio["path"]},  # inp_ref
            {"__type__": "update", "value": default_audio["text"]},  # prompt_text
        )
    else:
        return (
            {"__type__": "update", "choices": audio_choices, "value": ""},  # audio_dropdown
            {"__type__": "update", "value": None},  # inp_ref
            {"__type__": "update", "value": ""},  # prompt_text
        )

def on_audio_change(character_name, audio_display_name):
    """音频选择改变时的回调函数"""
    if not character_name or not audio_display_name:
        return (
            {"__type__": "update", "value": None},  # inp_ref
            {"__type__": "update", "value": ""},  # prompt_text
        )
    
    audio_info = get_audio_info_by_display_name(character_name, audio_display_name, character_data, "中文")
    
    if audio_info:
        return (
            {"__type__": "update", "value": audio_info["path"]},  # inp_ref
            {"__type__": "update", "value": audio_info["text"]},  # prompt_text
        )
    else:
        return (
            {"__type__": "update", "value": None},  # inp_ref
...
with gr.Blocks(title="GPT-SoVITS WebUI", analytics_enabled=False, js=js, css=css) as app:
    ...
    with gr.Row():
        with gr.Column():
            ...
            # 角色选择区域
            with gr.Group():
                gr.Markdown(value=i18n("角色音频选择 (可选)"))
                with gr.Row():
    ...
    with gr.Group():
        ...
        character_dropdown.change(
            on_character_change,
            [character_dropdown],
            [audio_dropdown, inp_ref, prompt_text]
        )
        
        audio_dropdown.change(
            on_audio_change,
            [character_dropdown, audio_dropdown],
            [inp_ref, prompt_text]
        )
